...
def list_emotion_logs(
    ...
    logs = svc.list_logs(trade_id, user_id=current_user.id)
    return {"items": logs}
...
def create_emotion_log(
    ...
    svc = EmotionLogService(db)
    entry = svc.create_log(trade_id, payload, user_id=current_user.id)
    return entry
...
def delete_emotion_log(
    ...
    svc = EmotionLogService(db)
    svc.delete_log(trade_id, log_id, user_id=current_user.id)
    return None
