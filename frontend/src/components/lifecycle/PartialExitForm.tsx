  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="text-sm text-text-muted mb-1 block">Qty</label>
          <input
            type="number"
            value={qty}
        ...
        <div>
          <label className="text-sm text-text-muted mb-1 block">Exit Price</label>
          <input
            type="number"
            value={exitPrice}
      ...
      {estPnl !== null && (
        <div className={`text-sm font-data text-center py-1.5 rounded-lg ${estPnl >= 0 ? 'bg-profit-muted/30 text-profit' : 'bg-loss-muted/30 text-loss'}`}>
          Est. P&L: {estPnl >= 0 ? '+' : ''}{formatPrice(estPnl)}
        </div>
      )}

      <div>
        <label className="text-sm text-text-muted mb-1 block">Reason</label>
        <select
          value={exitReason}
