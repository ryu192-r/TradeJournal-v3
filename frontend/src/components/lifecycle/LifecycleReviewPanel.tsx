...
export function LifecycleReviewPanel({ trade }: LifecycleReviewPanelProps) {
  ...
  return (
    <div className="border-t border-border pt-4 mt-4">
      <div className="text-[10px] sm:text-[11px] text-text-muted uppercase tracking-wider mb-3">Lifecycle Intelligence</div>

      <div className="flex flex-wrap gap-1.5 mb-3">
        <button
          onClick={() => togglePanel('emotion')}
          className={`flex items-center gap-1 text-[11px] px-2.5 py-1.5 rounded-lg border transition-all cursor-pointer ${activePanel === 'emotion' ? 'border-amber-400/50 bg-amber-400/10 text-amber-400' : 'border-border text-text-muted hover:text-text-heading hover:border-text-muted'}`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-1 text-[11px] px-2.5 py-1.5 rounded-lg border transition-all cursor-pointer ${activePanel === 'partial_exit' ? 'border-accent/50 bg-accent-faint text-accent' : 'border-border text-text-muted hover:text-text-heading hover:border-text-muted'}`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-1 text-[11px] px-2.5 py-1.5 rounded-lg border transition-all cursor-pointer ${activePanel === 'grade' ? 'border-accent/50 bg-accent-faint text-accent' : 'border-border text-text-muted hover:text-text-heading hover:border-text-muted'}`}
        >
      ...
      {activePanel === 'emotion' && (
        <div className="border border-amber-400/20 rounded-xl p-3 sm:p-4 bg-bg-elevated/30 mb-3">
          <EmotionLogger tradeId={trade.id} onClose={() => setActivePanel(null)} />
        </div>
      )}

      {activePanel === 'partial_exit' && (
        <div className="border border-accent/20 rounded-xl p-3 sm:p-4 bg-bg-elevated/30 mb-3">
          <PartialExitForm
            tradeId={trade.id}
