...
import { Target, Loader2, Edit3 } from 'lucide-react'
    ...
    <div className="space-y-[var(--page-gap)] pb-[max(var(--page-py),env(safe-area-inset-bottom))]">
      ...
      <div className={`rounded-2xl border p-[var(--page-px)] animate-card-in ${pnlBg}`}>
        <div className="flex items-baseline justify-between flex-wrap gap-2">
          <div>
            <div className="text-[length:var(--text-xs)] text-text-muted uppercase tracking-wider font-display">P&L</div>
            ...
            {trade.r_multiple != null && (
              <div className={`text-sm font-data mt-1 ${Number(trade.r_multiple) >= 0 ? 'text-profit' : 'text-loss'}`}>
      ...
      <div className={CARD}>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-3">
          ...
          {trade.exit_price && trade.r_multiple != null && (
            <div className="flex justify-between py-1 border-b border-border/40 sm:border-0">
              <span className="text-[length:var(--text-sm)] text-text-muted">R-Multiple</span>
              <span className={`text-[length:var(--text-sm)] font-medium font-data ${Number(trade.r_multiple) >= 0 ? 'text-profit' : 'text-loss'}`}>
                {Number(trade.r_multiple).toFixed(2)}R
              </span>
            </div>
          )}
